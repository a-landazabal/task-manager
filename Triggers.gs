...
function setup() {
  ...
  if (!ss.getSheetByName(SHEET_ROOMS) || ss.getSheetByName(SHEET_ROOMS).getLastRow() <= 1)
    syncRooms();
  if (!ss.getSheetByName(SHEET_MEMBERS) || ss.getSheetByName(SHEET_MEMBERS).getLastRow() <= 1)
    syncMembers();
...
function createTrigger() {
  ...
  Logger.log(
    'トリガー設定完了（ポーリング + 日次ルーム同期 + 毎時メンバー同期 + Notion同期 + カレンダー同期）。',
  );
}
...
function onEdit(e) {
  ...
  // ダッシュボードフィルター変更
  if (
    name === SHEET_DASHBOARD &&
    e.range.getRow() === 2 &&
    (e.range.getColumn() === 2 || e.range.getColumn() === 6)
  ) {
    refreshDashboard();
    return;
  }
