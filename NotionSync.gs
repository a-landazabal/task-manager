  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (existingPage) {
      ...
      if (existingPage.status !== status) {
        updateNotionPageStatus(existingPage.id, status, description);
        updated++;
      }
    } else {
      // ── 新しいページを作成 ──
...
function createNotionPage(task) {
  const properties = {
    タスク名: {
      title: [{ text: { content: (task.status === STATUS_FINISHED ? '🟢 ' : '🟡 ') + (task.description.substring(0, 2000) || task.taskId) } }],
    },
    ステータス: {
  ...
  if (code !== 200) {
    Logger.log(
      `Notionページ作成エラー [${task.taskId}]: ${code} ${res.getContentText().substring(0, 200)}`,
    );
  }
  return code === 200;
}
...
function updateNotionPageStatus(pageId, status, description) {
  var emoji = status === STATUS_FINISHED ? '🟢 ' : '🟡 ';
  var title = emoji + (description || '');
  const payload = {
    properties: {
      タスク名: {
...
// ============================================================
// 一回限り — 既存Notionページに絵文字を追加
// ============================================================

function refreshAllNotionEmojis() {
  ...
  var notionPages = getNotionPages();
  var refreshed = 0;

  for (var i = 0; i < data.length; i++) {
    var taskId = String(data[i][0]);
    var description = String(data[i][3]);
    var status = String(data[i][10]);

    if (!taskId) continue;

    var page = notionPages[taskId];
    if (!page) continue;

    updateNotionPageStatus(page.id, status, description);
    refreshed++;

    if (refreshed % 3 === 0) Utilities.sleep(500);
  }

  Logger.log('絵文字リフレッシュ完了: ' + refreshed + '件');
}
